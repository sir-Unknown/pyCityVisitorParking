...
from pycityvisitorparking.provider import loader as _loader
...
async def test_update_reservation_not_supported() -> None:
    provider = _provider(_SequenceSession([]))
    with pytest.raises(ProviderError, match="not supported"):
        await provider.update_reservation("1", end_time=datetime(2026, 3, 25, 12, 0, tzinfo=UTC))
...
async def test_add_favorite_not_supported() -> None:
    provider = _provider(_SequenceSession([]))
    with pytest.raises(ProviderError, match="not supported"):
        await provider.add_favorite("AB1234")


async def test_remove_favorite_not_supported() -> None:
    provider = _provider(_SequenceSession([]))
    with pytest.raises(ProviderError, match="not supported"):
...
def test_format_provider_timestamp_utc_to_local_winter() -> None:
    p = _bare_provider()
    assert p._format_provider_timestamp(datetime(2026, 3, 25, 9, 0, tzinfo=UTC)) == "25-03-2026 10:00:00"


def test_format_provider_timestamp_utc_to_local_summer() -> None:
    p = _bare_provider()
    assert p._format_provider_timestamp(datetime(2026, 7, 1, 10, 0, tzinfo=UTC)) == "01-07-2026 12:00:00"
...
def test_extract_location_from_parameter_group() -> None:
    product = {
        "pdt_id": "BDABZRG_1317$abc",
        "pdt_parameter_groups": [
            {"pgr_parameters": [{"prr_label": "LOCATION", "prr_value": "BDA1317"}]}
        ],
    }
...
def test_extract_location_fallback_regex() -> None:
    assert _extract_location({"pdt_id": "TELBZRG_9001$xyz", "pdt_parameter_groups": []}) == "TEL9001"
